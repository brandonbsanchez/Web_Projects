    ...
    <?php
        ...
        if(!mysqli_stmt_prepare($statement, $sql)) {
            ...
            while($row = mysqli_fetch_assoc($result)) { //Each one is a store card
                echo '<div class="store_card" id="store_'.$row['item_id'].'">
                <h2>'.$row['name'].'</h2>
                <p>'.$row['description'].'</p>
                <p>'.$row['num_in_stock'].'</p>
                <p>$'.$row['unit_price'].'</p>
                <img src="Uploads/Item/'.$row['img_dest'].'" height=50px><br>
                <button class="edit_store" type="button" value='.$row['item_id'].'>Edit Item</button>
                <form method="POST" action="Includes/deletestore_inc.php">
                <button type="submit" name="delete_store" value='.$row['item_id'].'>Delete Item</button>
                </form>
                </div>';
            }
        }
